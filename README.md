...
极狐GitLab SaaS 无需用户安装，注册即可开启云端使用。极狐作为服务的提供方，负责SaaS服务的高可用架构、灾备恢复等维护；用户为服务等消费者，**最高权限为群组所有者(Group Owner)**。

注意:

开发人员(Developers) 可以查看、编辑、推送代码(不能直接推送main分支)，但不能合并代码，需要维护者(Maintainer) 或部门总监(Owner)进行合并。并且不能创建仓库。

维护者(Maintainer) 可以查看、编辑、推送代码(可以推送main分支)，可以合并代码，且可以创建仓库，但不能删除仓库。

部门总监(Owner) 可以查看、编辑、推送代码(可以推送main分支)，可以合并代码，可以创建仓库，可以删除仓库。
...
![](./docs/查看GitLab到期时间.jpg)

### 增加席位:

![](./docs/增加席位.jpg)

增加新成员的花费是按照团队整体的到期时间算的，例如团队从去年9月份开始租用GitLab，那么增加新成员的花费就是按照现在到团队整体的到期时间算的。
